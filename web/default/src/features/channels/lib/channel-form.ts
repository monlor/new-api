...
export const channelFormSchema = z
  .object({
    ...
    billing_type: z.number().min(0).max(2).optional(),
    ...
    // Content review (stored in settings JSON, all channel types)
    skip_content_review: z.boolean().optional(),
...
export function transformChannelToFormDefaults(
  ...
  if (channel.settings) {
    try {
      ...
      skipContentReview = parsed.skip_content_review === true
  ...
  return {
    ...
    billing_type: channel.billing_type ?? 0,
    ...
    skip_content_review: skipContentReview,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  // Content review skip (available for all channel types)
  settingsObj.skip_content_review = formData.skip_content_review === true
...
export function transformFormDataToCreatePayload(formData: ChannelFormValues): {
  ...
  const channel: Partial<Channel> = {
    ...
    billing_type: formData.billing_type ?? 0,
