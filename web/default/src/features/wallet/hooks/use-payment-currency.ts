/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import { useQuery } from '@tanstack/react-query'
import { DEFAULT_CURRENCY_CONFIG } from '@/stores/system-config-store'
import { useSystemConfig } from '@/hooks/use-system-config'
import { getTopupInfo } from '../api'
import { formatPaymentCurrency } from '../lib'

// ============================================================================
// Payment Currency Hook
// ============================================================================

/**
 * Returns a formatBalance helper that formats quota values using the
 * payment gateway's configured currency (e.g. CNY, USD).
 *
 * Results are cached via React Query so multiple components on the same
 * page share a single fetch.
 */
export function usePaymentCurrency() {
  const { currency } = useSystemConfig()

  const quotaPerUnit =
    currency?.quotaPerUnit && currency.quotaPerUnit > 0
      ? currency.quotaPerUnit
      : DEFAULT_CURRENCY_CONFIG.quotaPerUnit

  const { data: topupInfo } = useQuery({
    queryKey: ['topup-info'],
    queryFn: async () => {
      const res = await getTopupInfo()
      return res.success && res.data ? res.data : null
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
  })

  const paymentCurrency = topupInfo?.payment_currency ?? 'CNY'

  const formatBalance = (quota: number): string => {
    return formatPaymentCurrency(quota / quotaPerUnit, paymentCurrency)
  }

  return { formatBalance, paymentCurrency }
}
