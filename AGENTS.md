...
- **To pull upstream updates, use the `upstream-sync` skill** (`.agents/skills/upstream-sync/`): it assesses first (bug-fix-focused, channel/relay prioritized), reads this file to judge conflict impact, documents large conflicting changes as "not merging" in `FORK_SYNC_ASSESSMENT.md`, and only then selectively hand-ports. Never blind-`git merge upstream/main`.
