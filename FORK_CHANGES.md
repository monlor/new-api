...
- 系统日志「管理」内容不再直接展示 `METHOD /api/...`：系统日志页按 `other.op.action` 用 i18n 模板渲染（与使用日志审计列同一套 `renderAuditContent`）。审计中间件覆盖全部 Admin/Root 写路由：探测/预览类 POST（Waffo catalog、ratio_sync、fetch_models、部署连通性测试等）不记日志；其余写操作都有稳定 action + 英文兜底，并由覆盖测试锁定 `router/api-router.go` 不得漏映射

**涉及文件：** `controller/subscription.go`、`controller/subscription_payment_epay.go`、`controller/subscription_payment_stripe.go`、`controller/subscription_payment_waffo_pancake.go`、`controller/topup_stripe.go`、`controller/audit.go`、`model/subscription.go`、`model/subscription_allowed_models_test.go`、`model/provider_subscription.go`、`model/main.go`、`router/api-router.go`、`service/billing_session.go`、`service/channel_select.go`、`middleware/distributor.go`、`middleware/audit.go`、`dto/user_settings.go`、`common/str.go`、`web/default/src/features/subscriptions/`、`web/default/src/features/subscriptions/lib/model-value.ts`、`web/default/src/features/subscriptions/components/subscriptions-mutate-drawer.tsx`、`web/default/src/features/wallet/`、`web/default/src/features/system-settings/integrations/payment-settings-section.tsx`、`web/default/src/features/system-logs/`、`web/default/src/features/usage-logs/lib/format.ts`、`web/default/src/i18n/locales/*.json`
...
- Grok CLI 快速配置：Use API Key 弹窗 Grok CLI 标签页输出环境变量（`XAI_API_KEY` / `GROK_XAI_API_BASE_URL`）和扁平底表 `~/.grok/config.toml`（`[endpoints].xai_api_base_url` / `models_base_url`、`[models]`、`[model."<id>"]` 的 `context_window` / `max_completion_tokens` / `supports_reasoning_effort`）。`GROK_MODELS_BASE_URL` 不写进环境变量，对应配置文件字段是 `endpoints.models_base_url`。上下文/输出用 `model-limits.ts` 家族表；思考级别按家族表（GPT 含 `ultra`），未知家族跳过，不发明 `grok-4.6` 默认模型。不写嵌套 `[[model.x.reasoning_efforts]]`
- 模型上下文限额抽到 `web/default/src/features/keys/lib/model-limits.ts`，URL 规范化 / 推理 / 视觉检测抽到 `model-meta.ts`，Grok CLI / OpenCode / Pi / CC Switch 共用。Claude Sonnet/Opus/Fable 1M、Haiku 200k、GPT 5.4+ / GPT-6 为 1,050,000、Grok 500k、DeepSeek/GLM 1M

**涉及文件：** `web/default/src/routes/pricing/index.tsx`、定价/keys 相关前端组件（含 `web/default/src/features/keys/components/dialogs/{cc-switch-dialog,use-api-key-dialog}.tsx`、`web/default/src/features/keys/lib/{opencode-config,pi-config,grok-config,model-limits,model-meta}.ts`、`web/default/src/features/keys/lib/api-key-form.ts`、`web/default/src/features/keys/components/api-keys-mutate-drawer.tsx`）、`constant/context_key.go`、`common/constants.go`
